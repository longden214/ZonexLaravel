...
<section class="content">
    <div class="box box-primary">
        <form action="{{route('admin.blog.update',$blog->id)}}" id="form_post" method="POST" role="form"
                ...
                <div class="col-md-9">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h3 class="panel-title">Cập nhật tin tức</h3>
                        </div>
                        <div class="panel-body">
                            <div class="form-group">
                                <label for="">Tiêu đề</label>
                                <input name="title" type="text" id="name" class="form-control" value="{{$blog->title}}">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input name="summary" type="text" class="form-control" value="{{$blog->summary}}">
                            </div>
                            <div class="form-group">
                                <label for="">Nội dung</label>
                                <textarea id="content_post" name="content">{{$blog->content}}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <select name="cat_blog_id" form="form_post" id="input" class="form-control"
                                required="required">
                                @foreach ($catList as $cat_blog)
                                <option value="{{$cat_blog->id}}" {{$cat_blog->id==$blog->cat_blog_id?'selected':''}}>{{$cat_blog->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <div class="input-group">
                                <input type="text" class="form-control" id="image_big" name="image" value="{{$blog->image}}">
                                <span class="input-group-btn">
                                    <a class="btn btn-primary" data-toggle="modal" href='#modal-id-big'>Select!</a>
                                </span>
                            </div>
                            <img src="{{url('uploads')}}/{{$blog->image}}" alt="" id="show_img_big" width="100%" height="auto">
                        </div>
                    </div>
                    <div class="panel panel-default">
                        <div class="panel-heading">
                        ...
                        <div class="panel-body">
                            <div class="input-group">
                                <input type="text" class="form-control" id="image_small" name="thumbnail" value="{{$blog->thumbnail_image}}">
                                <span class="input-group-btn">
                                    <a class="btn btn-primary" data-toggle="modal" href='#modal-id-small'>Select!</a>
                                </span>
                            </div>
                            <img src="{{url('uploads')}}/{{$blog->thumbnail_image}}" alt="" id="show_img_small" width="100%" height="auto">
                        </div>
                    </div>
                    <div class="panel panel-default">
                        <div class="panel-heading">
...
<script type="text/javascript">
    ...
    $(function () {
    $('.select-tag').chosen();
    $('.select-tag').chosen().val({!! json_encode($blog->tags()->allRelatedIds()) !!}).trigger("chosen:updated");
    })
